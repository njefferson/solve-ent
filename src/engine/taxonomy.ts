...
export function stagesFor(problem: Problem): Stage[] {
  switch (problem.topic) {
    ...
    case 'FRACTIONS': {
      const flip: Stage[] = problem.upsideDown
        ? [
            {
              id: 'F0',
              kind: 'NUMERIC',
              ...
              gradesSigFigs: false,
              prompt:
                `${problem.rate.written} ${formatUnit(problem.rate.unit)} is ${problem.rate.label}. ` +
...
function roundEarlyAnswer(problem: Problem, solution: Solution): number | null {
  ...
  switch (problem.topic) {
    ...
    case 'FRACTIONS': {
      ...
      if (!problem.upsideDown) return null;
...
export const REMEDIES: {
  readonly [K in Remedy]: { readonly name: string; readonly how: string; readonly why: string };
} = {
  'A1-ISOLATE': {
    name: 'undoing an operation on both sides to get one letter on its own',
    how:
      'To get a letter on its own, undo what is being done to it on BOTH sides — ' +
      'a letter that is multiplying the one you want is a letter you divide by.',
    why:
      'An equation is a statement that two things are equal, and it stays true only if ' +
  ...
  'A2-PROPORTION': {
    ...
    how:
      'Write both ratios the same way round — what you have over what the recipe takes, ' +
      'what you want over what it makes — and then cross-multiply.',
    why:
      'A balanced equation fixes a ratio that holds however much you make, so scaling ' +
  ...
  'A3-UNITS': {
    ...
    how:
      'Write each conversion with the unit you are carrying on the BOTTOM so it cancels; ' +
      'if a unit does not cancel, that factor is upside down.',
    why:
      'A conversion factor is worth 1 either way up, so the units are the only thing ' +
  ...
  'A4-MAGNITUDE': {
    ...
    how:
      'Ask roughly how big the answer ought to be before writing it — ' +
      'out by a factor of ten is usually a decimal point rather than the method.',
    why:
      'A chemical amount has a size you can predict before you calculate, so a number ' +
