...
import { TOPIC_NAMES, laddersFor, solve, statedValues, type Problem, type Topic } from '../engine/problem.ts';
...
import { checkForUpdate, heldCaches, watchForUpdate } from './update.ts';
...
interface WorkingLine {
  ...
  /**
   * The question that step asked, word for word.
   *
   * A reader who wants to check what a step actually wanted had no way to look:
   * the wording went with the step. This is the STAGE's prompt, which the
   * reader has already seen and answered — never anything from the solution.
   */
...
function renderQuestion(problem: Problem, stage: Stage): void {
  ...
  if (stated.length > 0) {
    ...
    for (const value of stated) {
      ...
      const item = make('li', {});
      item.append(
        make('span', { class: 'given-symbol' }, `${value.symbol} = ${value.written}${unit === '' ? '' : ` ${unit}`}`),
        make('span', { class: 'given-label' }, value.label),
      );
      list.append(item);
    }
  ...
  if (prefs.oneStepAtATime) {
    ...
    reveal.addEventListener('click', () => {
      ...
      if (givens !== null) givens.hidden = false;
      reveal.remove();
    });
 ...
 */
function renderEntry(problem: Problem, stage: Stage, takeFocus: boolean = true): void {
...
function answer(entry: Parameters<typeof submit>[1]): void {
  ...
  if (result.classification.correct) {
    ...
    if (wrote !== '') run.working.push({ what: STEP_DID[stage.id] ?? SKILL_NAMES[stage.counter], wrote, asked: stage.prompt });
...
function renderDiagnosis(
  ...
  if (remedies.length > 0) {
    ...
    if (only !== undefined) {
      panel.append(
        make('p', { class: 'remedy' }, REMEDIES[only].how),
        make('p', { class: 'remedy-why' }, REMEDIES[only].why),
      );
    } else {
...
function renderDrillEntry(item: DrillItem, takeFocus: boolean = true): void {
...
function renderReport(): void {
  ...
  for (const button of document.querySelectorAll<HTMLButtonElement>('[data-reason]')) {
...
function wireReport(): void {
  ...
  $('#copy-report').addEventListener('click', () => {
    ...
    void navigator.clipboard?.writeText(text).then(
      () => say('The report is copied.'),
      () => say('Copying is not available here, so the lines can be typed out instead.'),
    );
  });
}
...
export function boot(storeForTests?: Store): void {
  ...
  const asker = $('#check-updates') as HTMLButtonElement;
