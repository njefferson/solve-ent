...
/** What to do about the panel on this load. */
export type NotesDecision =
  /** Show it. The reader has been here before and the version has moved. */
  | { readonly show: true }
  /**
   * Do not show it. `remember` is a version to store now rather than on
   * dismiss — set only for a first-time visitor, who is being marked as having
   * arrived rather than as having read anything.
   */
  | { readonly show: false; readonly remember: string | null };
 ...
 */
export function decideNotes(stored: string | null, running: string): NotesDecision {
  if (stored === null) return { show: false, remember: running };
  if (stored === running) return { show: false, remember: null };
  return { show: true };
}
