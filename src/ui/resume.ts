...
import { MAX_ROSTER_NUMBER, type Session } from '../engine/steps.ts';
import { TOPIC_NAMES, posesTier, type Topic } from '../engine/problem.ts';
import { VERSION } from '../version.ts';
import type { Store } from './prefs.ts';
import type { Clock } from '../engine/steps.ts';
 ...
 */
export function saveRun(store: Store, session: Session, clock: Clock): void {
  if (session.config.mode !== 'assignment' || session.finished) {
    clearRun(store);
    return;
  }
  const stored: Stored = { version: VERSION, savedAtMs: clock.now(), session };
  store.set(RESUME_KEY, JSON.stringify(stored));
}

/** Forget it. Called when a set finishes, and when a reader says to. */
export function clearRun(store: Store): void {
 ...
 */
function readsAsSession(value: unknown): value is Session {
  if (typeof value !== 'object' || value === null) return false;
  const s = value as Record<string, unknown>;
  const config = s['config'] as Record<string, unknown> | undefined;
  if (config === undefined) return false;
  if (config['mode'] !== 'assignment') return false;
  if (typeof config['assignmentKey'] !== 'string' || config['assignmentKey'] === '') return false;
  const topic = config['topic'];
  if (typeof topic !== 'string' || !(topic in TOPIC_NAMES)) return false;
  const tier = config['tier'];
  if (typeof tier !== 'number' || !posesTier(topic as Topic, tier)) return false;
  ...
  if ((s['problemIndex'] as number) >= count) return false;
  if (typeof s['stageAttempted'] !== 'boolean') return false;
  if (s['finished'] !== false) return false;
  if (!isCounts(s['wrongBySkill']) || !isCounts(s['wrongByClass'])) return false;
  return true;
}

/** Why an unfinished set could not be picked up. Never a bare "no". */
export type ResumeRefusal = 'NONE' | 'UNREADABLE' | 'STALE' | 'OTHER_VERSION';

export type ResumeReading =
  | { readonly kind: 'run'; readonly session: Session; readonly savedAtMs: number }
  | { readonly kind: 'none'; readonly why: ResumeRefusal };
 ...
 */
export function readRun(store: Store, clock: Clock): ResumeReading {
  const raw = store.get(RESUME_KEY);
  if (raw === null || raw === '') return { kind: 'none', why: 'NONE' };
