...
/** What a caller does when a newer version turns out to be ready. */
export interface UpdateSurface {
  /** Show the standing indicator. `take` releases the waiting worker. */
  offer(take: () => void): void;
  /** Hide it. Called if the worker goes away before the reader acts. */
  withdraw(): void;
}
 ...
 */
export async function heldCaches(): Promise<string[]> {
  try {
 ...
 */
export function watchForUpdate(surface: UpdateSurface): void {
  if (!canCache()) return;
  ...
  container.addEventListener('controllerchange', () => {
    if (reloading) return;
    if (!hadController && !expectingSwap) return;
    reloading = true;
  ...
  void container
    ...
    .then((registration) => {
       ...
       */
      if (registration.active !== null && registration.waiting !== null) offer(registration.waiting);
      ...
      registration.addEventListener('updatefound', () => {
        ...
        incoming.addEventListener('statechange', () => {
          if (incoming.state === 'installed' && replacing) offer(registration.waiting ?? incoming);
          if (incoming.state === 'redundant') surface.withdraw();
        });
      });
    })
    .catch(() => {
