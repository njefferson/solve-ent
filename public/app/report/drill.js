 ...
 */
export function readRun(attempts) {
    const notes = [];
    const seen = new Map();
    const named = new Set();
    ...
    attempts.forEach((attempt, index) => {
        ...
        if (errorClass === 'E-NEAR-UNACCOUNTED' || errorClass === 'E-UNCLASSIFIED')
            return;
...
function stopped(attempts, errorClass, lastIndex, skill) {
    let cleanSince = 0;
    for (let i = lastIndex + 1; i < attempts.length; i += 1) {
        const attempt = attempts[i];
        if (attempt.skill !== skill)
            continue;
        if (attempt.errorClass === errorClass)
            return false;
        cleanSince += 1;
    }
    return cleanSince >= STOPPED_AFTER_CLEAN;
}
function closingFor(attempts, seen, lastAt, skillOf) {
    ...
    if (nothingWentRight) {
        ...
        const last = [...attempts]
            .reverse()
            .map((a) => a.errorClass)
            .find((errorClass) => errorClass !== null);
    ...
    for (const errorClass of repeated) {
        ...
        if (at === undefined || skill === undefined)
            continue;
        const fix = whatFixesIt(errorClass);
    ...
    if (lines.length === 0) {
        ...
        if (nothingWentWrong)
            return ['Nothing went wrong in those. Stop whenever you like.'];
