...
import { DISTINGUISHABLE_RELATIVE, ORDER_OF_MAGNITUDE_LIMIT, COUNT_NEAR_LIMIT, SCINOT_TRIGGER_LOG10, } from './tolerance.js';
 ...
 */
function rotateFromSeed(items, seed) {
    ...
    const indexOf = (i) => (i - by + items.length * 2) % items.length;
 ...
 */
export function stagesFor(problem) {
        ...
        case 'REARRANGE': {
            ...
            if (rest.length >= 2) {
                stages.push({
                    ...
                    needsUnit: false,
                    gradesSigFigs: false,
                    prompt: `${shownSymbol(relation, problem.solveFor)} has to be separated from ` +
            ...
            return [
                {
                    ...
                    needsUnit: false,
                    gradesSigFigs: false,
                    prompt: `The recipe takes ${problem.from.written} mol of ${problem.fromName} and you have ` +
                ...
                },
                {
                    ...
                    needsUnit: true,
                    gradesSigFigs: true,
                    prompt: `Now scale the ${problem.toName} by that. Give it to ${problem.answerSigFigs} ` +
            ...
            return [
                {
                    ...
                    needsUnit: false,
                    gradesSigFigs: false,
                    prompt: `${problem.operation === 'MULTIPLY' ? 'Multiplying' : 'Dividing'} does one thing to the ` +
                ...
                },
                {
                    ...
                    needsUnit: false,
                    gradesSigFigs: false,
                    prompt: `And the two front numbers — ${problem.operation === 'MULTIPLY' ? 'multiply' : 'divide'} ` +
                ...
                },
                {
                    ...
                    needsUnit: false,
                    gradesSigFigs: false,
                    prompt: `Scientific notation wants the front number between 1 and 10. ` +
                ...
                },
                {
                    ...
                    gradesSigFigs: true,
                    prompt: `Write the whole answer in scientific notation, to ${problem.answerSigFigs} ` +
                        `significant figures.`,
                },
            ];
        case 'POWERS': {
            const stages = [
                {
                    ...
                    needsUnit: false,
                    gradesSigFigs: false,
                    prompt: problem.direction === 'POWER'
            ...
            return [
                {
                    ...
                    needsUnit: false,
                    gradesSigFigs: false,
                    prompt: `The first conversion is: ${first?.label ?? ''}. ` +
                ...
                },
                {
                    ...
                    needsUnit: true,
                    gradesSigFigs: true,
                    prompt: `Now finish the chain. Give the answer to ${problem.answerSigFigs} significant figures, ` +
                        `with the unit you are left with.`,
                },
            ];
        }
        case 'SIGFIGS': {
            const first = problem.operands[0];
            const mixed = problem.operation === 'ADD_THEN_MULTIPLY';
            const sumStage = mixed
                ? [
                    {
                        id: 'Gs',
                        kind: 'COUNT',
                        ...
                        gradesSigFigs: false,
                        prompt: `Add the first two first. The addition rule limits the last DECIMAL PLACE, ` +
...
function firstProducedUnit(problem) {
    const first = problem.factors[0];
    if (first === undefined)
        return NO_UNIT;
    return runChain(1, problem.start.unit, [first]).unit;
}
function buildOptions(seed, correctText, wrong) {
    const unique = [];
    for (const candidate of wrong) {
        if (candidate.text === correctText)
            continue;
        if (unique.some((u) => u.text === candidate.text))
            continue;
...
function rearrangeOptions(problem) {
    const relation = relationById(problem.relationId);
    const seed = `${problem.seed}|R1`;
    if (relation === undefined)
        return buildOptions(seed, '?', []);
    ...
    const of = (symbol) => shownSymbol(relation, symbol);
    const target = of(problem.solveFor);
    if (relation.shape === 'OFFSET') {
        const other = of((onLeft ? relation.right : relation.left)[0]);
     ...
     * says the same thing.
     */
    const divisor = (parts) => parts.length > 1 ? `(${parts.join(' × ')})` : (parts[0] ?? '?');
...
export function readEntry(text) {
    ...
    try {
        ...
        if (error instanceof SigFigError)
            return null;
        throw error;
    }
    const tail = match[2].trim();
    if (tail.length === 0)
        return { quantity, unit: null, unitText: '' };
    try {
...
const relativeClose = (a, b, tolerance) => {
    if (a === b)
        return true;
...
export function correctValueAt(problem, solution, stage) {
 ...
 */
function roundEarlyAnswer(problem, solution) {
    const figures = requiredSigFigs(problem, solution);
    const rounded = (value) => roundToSigFigs(value, figures);
    switch (problem.topic) {
        case 'REARRANGE': {
            const relation = relationById(problem.relationId);
            if (relation === undefined || relation.shape === 'OFFSET')
                return null;
        ...
        case 'SCINOT': {
            const raw = problem.operation === 'MULTIPLY'
                ...
                : problem.firstMantissa / problem.secondMantissa;
            const exponent = problem.operation === 'MULTIPLY'
        ...
        case 'UNITS': {
            ...
            for (const factor of problem.factors)
                running = rounded(running * factor.value);
            return running;
        }
        case 'SIGFIGS': {
            ...
            if (problem.operation === 'ADD_THEN_MULTIPLY') {
                ...
                const sumValue = (problem.operands[0]?.quantity.value ?? 0) + (problem.operands[1]?.quantity.value ?? 0);
 ...
 */
export function predictionsFor(problem, solution, stage) {
    const raw = [];
 ...
 */
export function choiceItemsFor(problem) {
    const set = choiceOptionsFor(problem);
    ...
    if (set === null)
        throw new RangeError(`${problem.topic} builds a choice stage and declares no options for it`);
    return set.items;
}
export function choiceOptionsFor(problem) {
    switch (problem.topic) {
        case 'REARRANGE':
...
const of = (errorClass, value) => ({
    errorClass,
    value,
    why: CLASS_MEANINGS[errorClass],
});
function numericPredictions(problem, solution, stage) {
    const out = [];
    switch (problem.topic) {
        case 'REARRANGE': {
            const relation = relationById(problem.relationId);
        ...
        case 'SCINOT': {
            ...
            if (stage.id === 'N4') {
                ...
                if (shift !== 0)
                    out.push(of('E-SCI-NORMALISE', value * 10 ** -shift));
            }
            break;
        }
        case 'POWERS': {
        ...
        case 'UNITS': {
            ...
            if (stage.id === 'U2') {
                ...
                if (first !== undefined)
                    out.push(of('E-UNIT-FACTOR-INVERTED', start / first.value));
                break;
            }
            if (stage.id === 'U3') {
        ...
        case 'SIGFIGS': {
            ...
            if (stage.id === 'G1') {
                ...
                if (zeros > 0)
                    out.push(of('E-SIG-COUNT-ZEROS', countSigFigs(first) + zeros));
                break;
            }
            if (stage.id === 'Gs') {
                ...
                const shortcut = Math.min(countSigFigs(problem.operands[0]), countSigFigs(problem.operands[1]));
    ...
    if (stage.gradesSigFigs) {
        ...
        if (early !== null)
            out.push(of('E-SIG-ROUND-EARLY', early));
 ...
 */
export function requiredSigFigs(problem, solution) {
    if (problem.topic !== 'SIGFIGS')
        return problem.answerSigFigs;
...
function leadingZeroCount(value) {
...
function ruleCounts(problem) {
    ...
    if (problem.operation === 'ADD_THEN_MULTIPLY') {
        ...
        return {
            ...
            raw: (values[0] + values[1]) * values[2],
        };
    }
    const raw = problem.operation === 'MULTIPLY'
        ? values.reduce((a, b) => a * b, 1)
    ...
    const wrong = problem.operation === 'MULTIPLY'
        ? coarsestPlaceCount(problem, raw)
        : Math.min(...problem.operands.map((o) => countSigFigs(o)));
    return { right, wrong, raw };
}
function coarsestPlaceCount(problem, raw) {
    let coarsest = -Infinity;
    for (const operand of problem.operands) {
        const q = operand.quantity;
        if (q.kind === 'measured')
            coarsest = Math.max(coarsest, q.reading.lastPlace);
        else if (q.kind === 'ambiguous')
            coarsest = Math.max(coarsest, q.low.lastPlace);
    }
    if (coarsest === -Infinity)
        return 1;
 ...
 * tiebreak.
 */
export function collisionsFor(problem, solution) {
    const found = [];
    ...
    for (const stage of stagesFor(problem)) {
        ...
        for (let i = 0; i < predictions.length; i += 1) {
            for (let j = i + 1; j < predictions.length; j += 1) {
                ...
                if ((a.sigFigs === undefined) !== (b.sigFigs === undefined))
                    continue;
                let clash = false;
                let shown = '';
...
function writtenSigFigs(quantity) {
    if (quantity.kind === 'measured')
        return quantity.reading.sigFigs;
    if (quantity.kind === 'ambiguous')
        return null;
    return null;
}
/**
 ...
 */
function entryMatches(quantity, candidate, sigFigs) {
    const readings = quantity.kind === 'exact'
        ? []
        : quantity.kind === 'measured'
            ? [quantity.reading.sigFigs]
            : [quantity.low.sigFigs, quantity.high.sigFigs];
    if (readings.length === 0)
        return relativeClose(quantity.value, candidate, DISTINGUISHABLE_RELATIVE);
 ...
 */
function entryIsCorrect(quantity, candidate, sigFigs) {
    const readings = quantity.kind === 'exact'
        ? []
        : quantity.kind === 'measured'
            ? [quantity.reading.sigFigs]
 ...
 */
export function classify(problem, solution, stage, entry) {
    const predicted = predictionsFor(problem, solution, stage);
    const base = { stage: stage.id, collision: false, logError: null, matched: [] };
    if (stage.kind === 'CHOICE') {
    ...
    const logError = stage.kind === 'COUNT' || read.quantity.value === 0 || correctValue === 0 || !sameSign
        ? null
    ...
    for (const prediction of predicted.predictions) {
        if (prediction.value === undefined)
            continue;
        if (prediction.sigFigs !== undefined)
            continue;
        if (entryMatches(read.quantity, prediction.value, required))
 ...
 */
export function remediesFor(errorClass, logError) {
    const out = [];
    switch (errorClass) {
        ...
        case 'E-NEAR-UNACCOUNTED':
            ...
            if (logError !== null && Math.abs(logError) >= SCINOT_TRIGGER_LOG10)
                out.push('A4-MAGNITUDE');
            break;
        case 'E-UNCLASSIFIED':
            out.push('A4-MAGNITUDE');
 ...
 * scratch paper.
 */
export function correctEntryFor(problem, solution, stage, scratchSigFigs) {
    if (stage.kind === 'CHOICE') {
        const set = choiceOptionsFor(problem);
        if (set === null)
            throw new RangeError(`${problem.topic} has a choice stage and declares no options for it`);
        return { kind: 'choice', option: set.correct };
    }
    const value = correctValueAt(problem, solution, stage) ?? 0;
    if (stage.kind === 'COUNT')
        return { kind: 'text', text: String(Math.round(value)) };
