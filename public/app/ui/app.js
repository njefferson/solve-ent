...
function answerField() {
    return document.querySelector('#answer, #drill-answer');
}
/**
 ...
 */
function renderStepPlace(problem, stage) {
...
function renderDiagnosis(errorClass, why, logError, panel = $('#diagnosis'), stageKind = 'NUMERIC') {
    const nextAction = NEXT_ACTION[stageKind] ?? NEXT_ACTION['NUMERIC'];
    clear(panel);
    panel.hidden = false;
...
function takeAssignment() {
    const key = $('#assignment-key').value.trim();
    const roster = Number($('#roster-number').value.trim());
    const note = $('#assignment-note');
    const complain = (what) => {
